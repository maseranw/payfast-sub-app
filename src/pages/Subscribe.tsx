...
const Subscribe = () => {
  ...
  const handleSubscribe = async (plan: SubscriptionPlan) => {
    ...
    try {
      ...
      const paymentData: PaymentData = {
        ...
        subscription_type: '1',
        billing_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 30 days from now
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      const payfast = new PayFastService()
      await payfast.cancelSubscription(subscription.payfast_token, subscription.id)
  ...
  const handlePauseSubscription = async () => {
    if (!subscription || !subscription.payfast_token) {
      toast.error('Cannot pause subscription: missing PayFast token')
      return
    }
    ...
    try {
      const payfast = new PayFastService()
      await payfast.pauseSubscription(subscription.payfast_token)
  ...
  const handleUnpauseSubscription = async () => {
    if (!subscription || !subscription.payfast_token) {
      toast.error('Cannot unpause subscription: missing PayFast token')
      return
    }
    ...
    try {
      const payfast = new PayFastService()
      await payfast.unpauseSubscription(subscription.payfast_token)
      ...
      {subscription && (
        <div className="mb-8 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Current Subscription</h2>
            ...
            <div className="flex flex-wrap gap-2 sm:space-x-3 sm:gap-0">
              <button
                ...
                className="flex items-center px-3 sm:px-4 py-2 border border-yellow-300 rounded-lg text-yellow-700 hover:bg-yellow-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                {actionLoading ? (
              ...
              <button
                ...
                disabled={actionLoading}
                className="flex items-center px-3 sm:px-4 py-2 border border-green-300 rounded-lg text-green-700 hover:bg-green-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                {actionLoading ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ) : (
        ...
        {plans.map((plan) => (
          ...
          >
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 capitalize">per {plan.billing_cycle}</p>
            </div>
            ...
            <button
              ...
              disabled={loadingPlanId === plan.id || subscription?.plan_id === plan.id}
